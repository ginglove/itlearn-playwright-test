...
test("TC_EXAM_001: Automate Exam Creation", async ({page}) => {
    ...
    const endDate = page.locator('div.grid.grid-cols-7.gap-1.justify-items-center.mb-4 button').nth(9);
    ...
    await examPolicy.selectOption({label: "Warn & Lock — Warn then auto-submit after threshold"});
    const tabSwitch = page.locator('input[type="number"]').nth(1);
    ...
    const examCard = page.locator('div[class="glass-card p-6 flex flex-col h-full relative group"]').nth(0);
    const examCardTitle = examCard.locator('h3');
    await expect(examCardTitle).toHaveText("[VMH] Bài Kiểm Tra Playwright 15 Phút");
    await expect(examCardTitle).toBeVisible();

    // Edit the exam card (Attempt value)
    const editButton = examCard 
    .getByRole('button', {name: 'Edit Settings'});
    await editButton.click();

    const editAttempt = page.locator('input[class="premium-input"]').nth(2);
    await editAttempt.fill('10');
